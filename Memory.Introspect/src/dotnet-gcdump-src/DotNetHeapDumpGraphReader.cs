...
{
    private int _maxNodeCount = 10_000_000;
    /// <summary>
    ...
    public DotNetHeapDumpGraphReader(TextWriter log, int maxNodeCount)
    {
        m_log = log;
        if (maxNodeCount < 10_000_000) maxNodeCount = 10_000_000;
...
#pragma warning restore SYSLIB1045 // Convert to 'GeneratedRegexAttribute'.
        ...
        for (; ; )
        {
            ...
            if (m_graph.NodeCount >= _maxNodeCount)
            {
                ...
                m_log.WriteLine("[WARNING]: Exceeded max node count {0}. Processed {1}/{2} nodes with {3} node bulk events to go.",
                    _maxNodeCount, m_curNodeIdx, m_curNodeBlock.Count, m_nodeBlocks.Count);
                break;
            }
        }
